...
var compileTplReg = [
//if 语句
[/<!--{if (.+?)}-->/gi, '<?php if ($1) { ?>'], [/<!--{\/if}-->/gi, '<?php } ?>'], [/<!--{else}-->/gi, '<?php } else { ?>'], [/<!--{elseif (.*?)}-->/gi, '<?php } elseif ($1){ ?>'],

//echo 语句
[/{\$G_(.*?)}/gi, '<?php global $$$1; echo $$$1; ?>'], //快捷输出global变量
[/{\$lang_(.*?)}/gi, '<?php echo $language["$1"]; ?>'], [/{_(.*?)}/gi, '<?php echo $1; ?>'], [/{=(.*?)}/gi, '<?php echo $1; ?>'], [/{echo (.*?)}/gi, '<?php echo $1; ?>'], [/{eval (.*?)}/gi, '<?php $1; ?>'], [/{\$(.*?)}/gi, '<?php echo $$$1; ?>'],

//foreach for 语句
[/<!--{loop (.*?) (.*?) (.*?)}-->/gi, '<?php if(is_array($1)) { foreach($1 as $2=>$3) { ?>'], [/<!--{\/loop}-->/gi, '<?php } } ?>'], [/<!--{for (.*?)}-->/gi, '<?php for($1) { ?>'], [/<!--{\/for}-->/gi, '<?php } ?>'], [/\t/gi, ''],

//自定义方法
[/<!--{startBlock (.*?)}-->/gi, '<?php $this->startBlock($1); ?>'], [/<!--{endBlock}-->/gi, '<?php $this->endBlock(); ?>'],
